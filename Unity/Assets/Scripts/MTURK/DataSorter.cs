...
public struct UserComparisonData {
    public string UserId;
    public int Hit;
    public int ActualQInd;
    public int Answer;
...
public class DataSorter : MonoBehaviour {
        ...
        ParseData(); //males, nativeAll
         ...
         for (int p = 0; p < 5; p++) {
             ...
             for (int a = 0; a < 2; a++) {
                 ...
                 for (int e = 0; e < 4; e++) {
                     sw.WriteLine(GetAnimName(a) + " " + GetEffortName(e));                    
                      ClassifyData(sw, p, a, e);
                      if (e == 0 || e == 2) {
                          sw.WriteLine();
                          sw.WriteLine();
                      }
        ...
        */
            using (var reader = new StreamReader(File.OpenRead("userComparisonDataUnsorted.txt"))) {
                ...
                while (!reader.EndOfStream) {
                    ...
                    ud.ActualQInd = (ud.Hit - 1)*8 + int.Parse(tokens[2]) - 1;
    ...
    private string GetEffortName(int e) {
        switch (e) {
            case 0:
                return "Space";
            case 1:
    ...
    //personality = [0 4] as OCEAN, animInd = [0 1] as point pick, effort = [0 3] as space weight time flow
    public void ClassifyData(StreamWriter sw,  int personality, int animInd, int effort) {
        int qStart, qEnd;        
